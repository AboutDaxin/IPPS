...
# 读取excel的worker
data_worker = pd.read_excel(filename, sheet_name=2)
# 用于存储格式化后的总数据
format_datas = []
# 进行格式化
# 按行遍历
for i in range(data_worker.shape[0]):
    # 临时存储格式化后的行数据
    format_data = []
    # 在每行中按列遍历
    for j in range(data_worker.shape[1]):
        # 字符串表格化
        if j == 1:
            # 以‘,’为分隔符，将字符串列表化
            lst1 = data_worker.values[i, j][1:-1]
            lst2 = str(lst1).split(',')
            # 将列表元素转为int
            lst3 = list(map(int, lst2))
            format_data.append(lst3)
        # 整型数据正常处理
            ...
            format_data.append(data_worker.values[i, j])
    # 插入格式化后的整行数据
    format_datas.append(format_data)
Worker = format_datas

# 读取excel的instrument
data_instrument = pd.read_excel(filename, sheet_name=3)
# 用于存储格式化后的总数据
format_datas = []
# 进行格式化
# 按行遍历
for i in range(data_instrument.shape[0]):
    # 临时存储格式化后的行数据
    format_data = []
    # 在每行中按列遍历
    for j in range(data_instrument.shape[1]):
        # 字符串表格化
        if j == 1:
            # 以‘,’为分隔符，将字符串列表化
            lst1 = data_instrument.values[i, j][1:-1]
            lst2 = str(lst1).split(',')
            # 将列表元素转为int
            lst3 = list(map(int, lst2))
            format_data.append(lst3)
        # 整型数据正常处理
            ...
            format_data.append(data_instrument.values[i, j])
    # 插入格式化后的整行数据
    format_datas.append(format_data)
Instrument = format_datas
