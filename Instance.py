...
def Instance():
    ...
    for n in range(test_number):
        ...
        # 实例化一组工人
        worker_list = []
        for s in Input_outside.Worker:
            worker_list.append(Worker(s[0], s[1]))

        # 实例化一组设备
        instrument_list = []
        for s in Input_outside.Instrument:
            instrument_list.append(Instrument(s[0], s[1]))

        # 将该实例加入problems列表（第三个参数为限定时间）
        problems.append(Problem(task_list, task_group_list, station_list, worker_list, instrument_list, 99999))
        # 实例化一个gp，n为第几次运行
        gp = GP(number=n)
        # 执行该问题
